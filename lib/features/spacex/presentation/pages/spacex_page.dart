  ...
  Widget build(BuildContext context) {
    return MultiBlocProvider(
      providers: [
        BlocProvider(
          create: (context) =>
              getIt<LatestLaunchBloc>()..add(const LatestLaunchEvent.fetch()),
        ),
        BlocProvider(
          create: (context) =>
              getIt<HistoryBloc>()..add(const HistoryEvent.fetch()),
        ),
          ...
          slivers: [
            ...
            BlocBuilder<HistoryBloc, HistoryState>(
              builder: (context, state) {
                return switch (state) {
                  HistoryInitial() => SliverFillRemaining(
                    hasScrollBody: false,
                    child: Center(child: Text(l10n.initializingControl)),
                  ),
                  HistoryLoading() => const _HistoryShimmerLoader(),
                  HistoryError(message: final msg) => SliverFillRemaining(
                    hasScrollBody: false,
                    child: Center(child: Text(l10n.telemetryError(msg))),
                  ),
          ...
          children: [
            ...
            AnimatedSize(
              ...
              child: switch (state) {
                ...
                LatestLaunchError(message: final msg) => _ErrorPlaceholder(
                  message: msg,
                  l10n: l10n,
                ),
...
void _showLaunchDetails(BuildContext context, LaunchEntity launch) {
  final l10n = AppLocalizations.of(context)!;
  final colorScheme = Theme.of(context).colorScheme;
